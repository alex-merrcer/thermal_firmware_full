 ...
 * @brief Low-power runtime state machine and power policy orchestration.
 */
...
static void low_power_runtime_register_pm_clients(void)
{
    static const app_pm_client_t s_display_pm_client = {
        ...
        0,
        0,
        low_power_runtime_pm_display_prepare_standby
    };
    static const app_pm_client_t s_gpio_pm_client = {
        "gpio_pm",
        0,
        low_power_runtime_pm_gpio_prepare_stop,
        low_power_runtime_pm_gpio_restore_stop,
        low_power_runtime_pm_gpio_prepare_standby
    };
    static const app_pm_client_t s_thermal_pm_client = {
        "thermal",
        0,
        0,
        low_power_runtime_pm_thermal_restore_stop,
        0
    };
    static const app_pm_client_t s_watchdog_pm_client = {
        "watchdog",
        low_power_runtime_pm_watchdog_can_sleep,
        0,
        low_power_runtime_pm_watchdog_restore_stop,
        0
    };
    ...
    /* Host prepare-stop/prepare-standby still stays on the service-bus path
     * because it needs timeout/ack semantics. Registry takes local PM hooks first. */
...
/**
 * @brief Send a low-power prepare command through the app service bus and wait for the reply.
 * @param cmd_id Command id for the target low-power preparation stage.
 * @param timeout_ms Coordination timeout in milliseconds.
 * @return 1 when preparation succeeds; otherwise 0.
 */
