#ifndef REDPIC1_THERMAL_H
#define REDPIC1_THERMAL_H

#include <stdint.h>
#include "lcd_init.h"

/*
 * redpic1_thermal.h
 * Thermal capture/display public interface.
 */

/* Thermal diagnostic mode selection. */
#define REDPIC1_THERMAL_DIAG_MODE_NORMAL        0U
#define REDPIC1_THERMAL_DIAG_MODE_TEST_PATTERN  1U

#ifndef REDPIC1_THERMAL_DIAG_MODE
    #define REDPIC1_THERMAL_DIAG_MODE REDPIC1_THERMAL_DIAG_MODE_NORMAL
#endif

/* Stage 6L: thermal latency/ghosting diagnostics and mitigation. */
#ifndef REDPIC1_THERMAL_STAGE6L_ENABLE
    #define REDPIC1_THERMAL_STAGE6L_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGE6L_1_ENABLE
    #define REDPIC1_THERMAL_STAGE6L_1_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGE6L_1_WINDOW_ENABLE
    #define REDPIC1_THERMAL_STAGE6L_1_WINDOW_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGE6L_1_FILTER_ENABLE
    #define REDPIC1_THERMAL_STAGE6L_1_FILTER_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGEP5_ENABLE
    #define REDPIC1_THERMAL_STAGEP5_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGEP6_ENABLE
    #define REDPIC1_THERMAL_STAGEP6_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGEP7_ENABLE
    #define REDPIC1_THERMAL_STAGEP7_ENABLE 0U
#endif

#ifndef REDPIC1_THERMAL_STAGEP7_NORMAL_EMA_ALPHA
    #define REDPIC1_THERMAL_STAGEP7_NORMAL_EMA_ALPHA 0.25f
#endif

#ifndef REDPIC1_THERMAL_STAGEP7_NORMAL_MAX_STEP_C
    #define REDPIC1_THERMAL_STAGEP7_NORMAL_MAX_STEP_C 0.75f
#endif

#ifndef REDPIC1_THERMAL_STAGEP7_HIGH_MOTION_EMA_ALPHA
    #define REDPIC1_THERMAL_STAGEP7_HIGH_MOTION_EMA_ALPHA 0.85f
#endif

#ifndef REDPIC1_THERMAL_STAGEP7_HIGH_MOTION_MAX_STEP_C
    #define REDPIC1_THERMAL_STAGEP7_HIGH_MOTION_MAX_STEP_C 3.0f
#endif

#ifndef REDPIC1_THERMAL_STAGE6L_2_ENABLE
    #define REDPIC1_THERMAL_STAGE6L_2_ENABLE REDPIC1_THERMAL_STAGEP5_ENABLE
#endif

#ifndef REDPIC1_THERMAL_STAGE6L_3_ENABLE
    #define REDPIC1_THERMAL_STAGE6L_3_ENABLE REDPIC1_THERMAL_STAGEP6_ENABLE
#endif

#if (REDPIC1_THERMAL_STAGE6L_2_ENABLE != REDPIC1_THERMAL_STAGEP5_ENABLE)
    #error "P5 must stay aligned with REDPIC1_THERMAL_STAGE6L_2_ENABLE."
#endif

#if (REDPIC1_THERMAL_STAGE6L_3_ENABLE != REDPIC1_THERMAL_STAGEP6_ENABLE)
    #error "P6 must stay aligned with REDPIC1_THERMAL_STAGE6L_3_ENABLE."
#endif

#if (REDPIC1_THERMAL_STAGE6L_ENABLE == 0U)
    #if (REDPIC1_THERMAL_STAGE6L_1_ENABLE != 0U) || \
        (REDPIC1_THERMAL_STAGE6L_2_ENABLE != 0U) || \
        (REDPIC1_THERMAL_STAGE6L_3_ENABLE != 0U)
        #error "REDPIC1_THERMAL_STAGE6L_ENABLE=0 requires 6L-1/6L-2/6L-3 to stay disabled."
    #endif
#endif

#if ((REDPIC1_THERMAL_STAGE6L_2_ENABLE != 0U) || \
     (REDPIC1_THERMAL_STAGE6L_3_ENABLE != 0U)) && \
    (REDPIC1_THERMAL_STAGE6L_1_ENABLE == 0U)
    #error "6L-2 and 6L-3 require REDPIC1_THERMAL_STAGE6L_1_ENABLE=1."
#endif

#if (REDPIC1_THERMAL_STAGEP7_ENABLE != 0U) && \
    ((REDPIC1_THERMAL_STAGEP5_ENABLE == 0U) || \
     (REDPIC1_THERMAL_STAGE6L_1_ENABLE == 0U) || \
     (REDPIC1_THERMAL_STAGE6L_1_WINDOW_ENABLE == 0U))
    #error "P7 requires P5/6L-2 and the 6L-1 display-window path."
#endif

#if (REDPIC1_THERMAL_STAGE6L_ENABLE != 0U) && \
    (REDPIC1_THERMAL_STAGE6L_1_ENABLE == 0U) && \
    ((REDPIC1_THERMAL_STAGE6L_1_WINDOW_ENABLE != 1U) || \
     (REDPIC1_THERMAL_STAGE6L_1_FILTER_ENABLE != 1U))
    #error "6L-1 window/filter sub-switches only take effect when REDPIC1_THERMAL_STAGE6L_1_ENABLE=1."
#endif

/* Stage 6R: thermal real pipeline latency optimization. */
#ifndef REDPIC1_THERMAL_STAGE6R_ENABLE
    #define REDPIC1_THERMAL_STAGE6R_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGE6R_1_ENABLE
    #define REDPIC1_THERMAL_STAGE6R_1_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGE6R_2_ENABLE
    #define REDPIC1_THERMAL_STAGE6R_2_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGE6R_3_ENABLE
    #define REDPIC1_THERMAL_STAGE6R_3_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGE6R_D1_ENABLE
    #define REDPIC1_THERMAL_STAGE6R_D1_ENABLE 0U
#endif

#ifndef REDPIC1_THERMAL_STAGE6R_D1_STRIPE_ROWS
    #define REDPIC1_THERMAL_STAGE6R_D1_STRIPE_ROWS 1U
#endif

/* Stage P8-R: render-path optimization experiments and mainline replacement. */
#ifndef REDPIC1_THERMAL_STAGEP8R1_BLOCK_NEAREST_ENABLE
    #define REDPIC1_THERMAL_STAGEP8R1_BLOCK_NEAREST_ENABLE 0U
#endif

#ifndef REDPIC1_THERMAL_STAGEP8R2_SMALL_AREA_ENABLE
    #define REDPIC1_THERMAL_STAGEP8R2_SMALL_AREA_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGEP8R2_AREA_WIDTH
    #define REDPIC1_THERMAL_STAGEP8R2_AREA_WIDTH 160U
#endif

#ifndef REDPIC1_THERMAL_STAGEP8R2_AREA_HEIGHT
    #define REDPIC1_THERMAL_STAGEP8R2_AREA_HEIGHT 120U
#endif

#ifndef REDPIC1_THERMAL_STAGEP8R3_MAP_TABLE_ENABLE
    #define REDPIC1_THERMAL_STAGEP8R3_MAP_TABLE_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGEP8R4_FIXED_BILINEAR_ENABLE
    #define REDPIC1_THERMAL_STAGEP8R4_FIXED_BILINEAR_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGEP8R5_SWAPPED_LUT_ENABLE
    #define REDPIC1_THERMAL_STAGEP8R5_SWAPPED_LUT_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_STAGEP8R6_REPEAT_FILL_ENABLE
    #define REDPIC1_THERMAL_STAGEP8R6_REPEAT_FILL_ENABLE 0U
#endif

#ifndef REDPIC1_THERMAL_STAGEP8R7_TASK_PRIO_PROFILE
    #define REDPIC1_THERMAL_STAGEP8R7_TASK_PRIO_PROFILE 0U
#endif

/* Deprecated: superseded by the P8-R render-path experiment switches. */
#ifndef REDPIC1_THERMAL_LOW_COST_RENDER_TEST_ENABLE
    #define REDPIC1_THERMAL_LOW_COST_RENDER_TEST_ENABLE 0U
#endif

#ifndef REDPIC1_THERMAL_32HZ_AB_TEST_ENABLE
    #define REDPIC1_THERMAL_32HZ_AB_TEST_ENABLE 0U
#endif

#ifndef REDPIC1_THERMAL_STAGE6R_I1_ENABLE
    #define REDPIC1_THERMAL_STAGE6R_I1_ENABLE 0U
#endif

#ifndef REDPIC1_THERMAL_DROP_EXPIRED_FRAME_ENABLE
    #define REDPIC1_THERMAL_DROP_EXPIRED_FRAME_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_DROP_EXPIRED_FRAME_MIN_MS
    #define REDPIC1_THERMAL_DROP_EXPIRED_FRAME_MIN_MS 120UL
#endif

#ifndef REDPIC1_THERMAL_OVERLAY_DIRTY_ONLY_ENABLE
    #define REDPIC1_THERMAL_OVERLAY_DIRTY_ONLY_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_KEY2_SEND_ESP_ENABLE
    #define REDPIC1_THERMAL_KEY2_SEND_ESP_ENABLE 1U
#endif

#ifndef REDPIC1_THERMAL_PAUSE_SEND_ESP_FEATURE_ENABLE
    #define REDPIC1_THERMAL_PAUSE_SEND_ESP_FEATURE_ENABLE REDPIC1_THERMAL_KEY2_SEND_ESP_ENABLE
#endif

#ifndef REDPIC1_THERMAL_KEY2_ENTRY_GUARD_MS
    #define REDPIC1_THERMAL_KEY2_ENTRY_GUARD_MS 220UL
#endif

#if (REDPIC1_THERMAL_STAGE6R_ENABLE == 0U)
    #if (REDPIC1_THERMAL_STAGE6R_1_ENABLE != 0U) || \
        (REDPIC1_THERMAL_STAGE6R_2_ENABLE != 0U) || \
        (REDPIC1_THERMAL_STAGE6R_3_ENABLE != 0U) || \
        (REDPIC1_THERMAL_STAGE6R_D1_ENABLE != 0U) || \
        (REDPIC1_THERMAL_STAGE6R_I1_ENABLE != 0U)
        #error "REDPIC1_THERMAL_STAGE6R_ENABLE=0 requires all 6R sub-stages to stay disabled."
    #endif
#endif

#if (REDPIC1_THERMAL_STAGE6R_2_ENABLE != 0U) && \
    (REDPIC1_THERMAL_STAGE6R_1_ENABLE == 0U)
    #error "6R-2 requires REDPIC1_THERMAL_STAGE6R_1_ENABLE=1."
#endif

#if (REDPIC1_THERMAL_STAGE6R_3_ENABLE != 0U) && \
    ((REDPIC1_THERMAL_STAGE6R_1_ENABLE == 0U) || \
     (REDPIC1_THERMAL_STAGE6R_2_ENABLE == 0U))
    #error "6R-3 requires REDPIC1_THERMAL_STAGE6R_1_ENABLE=1 and REDPIC1_THERMAL_STAGE6R_2_ENABLE=1."
#endif

#if (REDPIC1_THERMAL_STAGE6R_D1_ENABLE != 0U) && \
    ((REDPIC1_THERMAL_STAGE6R_1_ENABLE == 0U) || \
     (REDPIC1_THERMAL_STAGE6R_2_ENABLE == 0U) || \
     (REDPIC1_THERMAL_STAGE6R_3_ENABLE == 0U))
    #error "6R-D1 requires 6R-1/6R-2/6R-3 to be enabled first."
#endif

#if (REDPIC1_THERMAL_STAGE6R_D1_ENABLE != 0U) && \
    (REDPIC1_THERMAL_STAGE6R_D1_STRIPE_ROWS == 0U)
    #error "6R-D1 stripe rows must be greater than zero."
#endif

#if (REDPIC1_THERMAL_STAGEP8R4_FIXED_BILINEAR_ENABLE != 0U) && \
    (REDPIC1_THERMAL_STAGEP8R3_MAP_TABLE_ENABLE == 0U)
    #error "P8-R4 requires REDPIC1_THERMAL_STAGEP8R3_MAP_TABLE_ENABLE=1."
#endif

#if (REDPIC1_THERMAL_STAGEP8R6_REPEAT_FILL_ENABLE != 0U) && \
    (REDPIC1_THERMAL_STAGEP8R1_BLOCK_NEAREST_ENABLE == 0U)
    #error "P8-R6 repeat fill requires REDPIC1_THERMAL_STAGEP8R1_BLOCK_NEAREST_ENABLE=1."
#endif

#if (REDPIC1_THERMAL_STAGEP8R2_SMALL_AREA_ENABLE != 0U) && \
    ((REDPIC1_THERMAL_STAGEP8R2_AREA_WIDTH == 0U) || \
     (REDPIC1_THERMAL_STAGEP8R2_AREA_WIDTH > LCD_W) || \
     (REDPIC1_THERMAL_STAGEP8R2_AREA_HEIGHT == 0U) || \
     (REDPIC1_THERMAL_STAGEP8R2_AREA_HEIGHT > (LCD_H - 20U)))
    #error "P8-R2 area must stay within the thermal display window."
#endif

#if (REDPIC1_THERMAL_STAGEP8R7_TASK_PRIO_PROFILE > 2U)
    #error "P8-R7 task priority profile supports only 0, 1 or 2."
#endif

#if (REDPIC1_THERMAL_STAGE6R_I1_ENABLE != 0U) && \
    ((REDPIC1_THERMAL_STAGE6R_1_ENABLE == 0U) || \
     (REDPIC1_THERMAL_STAGE6R_2_ENABLE == 0U) || \
     (REDPIC1_THERMAL_STAGE6R_3_ENABLE == 0U) || \
     (REDPIC1_THERMAL_STAGE6R_D1_ENABLE == 0U))
    #error "6R-I1 requires 6R-1/6R-2/6R-3/6R-D1 to be enabled first."
#endif

void redpic1_thermal_init(void);
void redpic1_thermal_bind_display_runtime(void);
void redpic1_thermal_step(void);
uint32_t redpic1_thermal_get_active_period_ms(void);
void redpic1_thermal_force_refresh(void);
void redpic1_thermal_handle_key(uint8_t key_value);
void redpic1_thermal_render_runtime_overlay(void);
uint8_t redpic1_thermal_runtime_overlay_visible(void);
void redpic1_thermal_suspend(void);
void redpic1_thermal_resume(void);
void redpic1_thermal_restore_bus_after_stop(void);
void redpic1_thermal_set_overlay_hold(uint8_t enabled);

#endif
