...
REM ============================================================
REM  Full firmware safe update script - NO pull / merge / rebase
...
set "COMMIT_MSG=%~1"
if "%COMMIT_MSG%"=="" (
    set "COMMIT_MSG=update: full firmware %date% %time%"
)
...
if not exist ".git\" (
    echo [ERROR] .git not found at firmware root. Run firmware_first_upload_no_conflict.cmd first.
    pause
    exit /b 1
)
...
if errorlevel 1 (
    echo [ERROR] This repo has no commit yet. Run firmware_first_upload_no_conflict.cmd first.
    pause
    exit /b 1
)
...
git status --porcelain --untracked-files=all > "%TEMP%\redpic1_full_git_status.txt"
for %%A in ("%TEMP%\redpic1_full_git_status.txt") do if %%~zA EQU 0 (
    echo.
...
echo.
echo [CONFIRM] This will commit and push local firmware changes only.
...
echo.
echo [OK] Full firmware update pushed successfully.
pause
exit /b 0
