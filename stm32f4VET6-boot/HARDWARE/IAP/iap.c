...
static uint8_t iap_txn_matches_current_app_commit(const OtaTxnRecord *txn,
...
static void iap_try_clear_stale_completed_txn(const BootInfoTypeDef *boot_info)
{
    OtaTxnRecord txn;
    ...
    txn_load(&txn);
    if (iap_txn_matches_current_app_commit(&txn, boot_info) == 0U)
    {
        return;
    }
...
static void iap_delay_ms_with_optional_watchdog(uint32_t total_ms, uint8_t feed_watchdog)
{
    uint32_t elapsed_ms = 0U;

    while (elapsed_ms < total_ms)
    {
        uint32_t step_ms = total_ms - elapsed_ms;
...
void iap_boot_entry(void)
{
    ...
    clear_reset_flags();
    boot_info_load(&boot_info);
    iap_try_clear_stale_completed_txn(&boot_info);
    ...
    if (reset_reason == BOOT_REASON_POWER || reset_reason == BOOT_REASON_IWDG)
    {
        ...
        iap_delay_ms_with_optional_watchdog(IAP_BOOT_SPLASH_HOLD_MS,
                                            (reset_reason == BOOT_REASON_IWDG) ? 1U : 0U);
    }
