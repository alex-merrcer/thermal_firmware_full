...
/*
 * Keil/ARMCC on STM32 may not provide __has_include, which previously caused
 * this file to silently fall back to the built-in development key even when a
 * local ota_security_secrets.h was present. That mismatch leads to AUTH HEXP
 * after OTA transfer completes.
 *
 * Production builds on ARMCC therefore prefer the local secrets header by
 * default. To intentionally use the development fallback on STM32, define
 * OTA_FORCE_DEV_AES_KEY in the project options before compiling.
 */
